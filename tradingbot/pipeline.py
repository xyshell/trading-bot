...
class BacktestPipeline(Pipeline):
    ...
    def _post(self, now: pd.Timestamp, strategy: Strategy):
        # TODO: execute orders elsewhere
        # # execute new orders + pending orders
        # self._mark_to_market(strategy)
        # orders = util.to_list(new_orders) + strategy.orders
        # for order in orders:
    ...
    def run(self, strategy: Strategy, **kwargs) -> None:
        ...
        Args:
            strategy (Strategy): execute strategy in backtest pipeline
        """
        start_tic = time.time()
...
class LivePipeline(Pipeline):
    ...
    def _prep(self, now: pd.Timestamp, strategy: Strategy) -> bool:
        ...
        # reflect balance
        if self._reflect_balance and not isinstance(strategy.exchange, FakeExchange) and (ticker := strategy.param.get("ticker")):
            n = 3
            while n > 0:
                try:
                    strategy.balance = strategy.exchange.reflect_balance(now, strategy.init_balance, ticker)
                except Exception as e:
                    strategy.logger.debug(f"Failed to reflect balance. due to {e!r}. Retrying {n=}.")
                    n -= 1
                    time.sleep(random.randint(1, 5))
                else:
                    break
            else:
                strategy.logger.error("Failed to reflect balance after retries. Delaying to next run.")
                msg = traceback.format_exc()
                strategy.logger.debug(f"{msg}")
                return False
    ...
    def run(self, strategy: Strategy, **kwargs):
        ...
        # reflect balance
        if self._reflect_balance and not isinstance(strategy.exchange, FakeExchange) and (ticker := strategy.param.get("ticker")):
            strategy.balance = strategy.exchange.reflect_balance(self._now_factory(), strategy.init_balance, ticker)
        ...
        try:
            for now in now_generator():
                ...
                if any(triggered):
                    ...
                    # call next
                    new_orders = strategy.next(context={"now": now, "trigger": triggered, "orders": strategy.orders})
