...
class Reporter:
    ...
    def get_portfolio_report(strategy: Strategy) -> pd.DataFrame:
        ...
        portfolio_report = portfolio_report.to_frame(f"NAV_{strategy.report_currency}")
    ...
    def get_transaction_report(strategy: Strategy) -> pd.DataFrame:
        if not strategy.transaction_history:
            ...
            # return pd.DataFrame(columns=list(field) + ["from_asset", "from_qty", "to_asset", "to_qty", "fee_asset", "fee_qty"])
    ...
    def get_trade_report(strategy: Strategy) -> pd.DataFrame:
        ...
        nav_col = f"NAV_{strategy.report_currency}"
