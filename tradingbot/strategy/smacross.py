...
from tradingbot.exchange import FakeExchange, CCXTExchange
...
class SMACross(Strategy):
    param = {"ticker": "USDT/BTC", "freq": "1m", "fast": 10, "slow": 30}
    ...
    def __init__(self, **kwargs):
        ...
        self.data = {
            "candlestick": Candlestick("binance", ticker=self.param['ticker'], freq=self.param['freq'], load_len=self.param["slow"] + 5)
        }
    ...
    def next(self):  # function to be called when triggered
        self.balance = self.balance.reflect(self.trader.exchange)  # reflect balance from exchange to self.balance
        ...
        if crossup:
            ...
            quote_asset, base_asset = self.param["ticker"].split("/")
            self.balance.convert(1.0, "PCTG", quote_asset, base_asset, trader=self.trader)  # convert [100] [percentage] of [USDT] to [BTC]
            ...
            self.balance.convert(1.0, "PCTG", base_asset, quote_asset, trader=self.trader)  # convert [100] [percentage] of [BTC] to [USDT]
        else:
    ...
    def plot(self):  # function to customize strategy plot
        fig = super().plot(engine="matplotlib", add_panels=1)
...
if __name__ == "__main__":
    ...
    # step 2: add strategy
    bot.add_strategy(SMACross(ticker="USDT/BNB"))
